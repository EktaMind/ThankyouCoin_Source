...
func ApplyGenesis(statedb *state.StateDB, g thankyoucoin.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g thankyoucoin.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g thankyoucoin.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
