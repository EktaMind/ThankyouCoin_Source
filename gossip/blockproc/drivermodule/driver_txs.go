...
import (
	...
	"github.com/kalibroida/ThankyouCoin_Node/thankyoucoin/genesis/driver/drivercall"
	"github.com/kalibroida/ThankyouCoin_Node/thankyoucoin/genesis/driver/driverpos"
	...
	netinitcall "github.com/kalibroida/ThankyouCoin_Node/thankyoucoin/genesis/netinit/netinitcalls"
...
func NewDriverTxGenesisTransactor(g thankyoucoin.Genesis) *DriverTxGenesisTransactor {
...
func (p *DriverTxPreTransactor) PopInternalTxs(block blockproc.BlockCtx, bs blockproc.BlockState, es blockproc.EpochState, sealing bool, statedb *state.StateDB) types.Transactions {
	...
	if sealing {
		...
		for oldValIdx := idx.Validator(0); oldValIdx < es.Validators.Len(); oldValIdx++ {
			...
			missed := thankyoucoin.BlocksMissed{
...
func (p *DriverTxListener) OnNewLog(l *types.Log) {
	...
	if l.Topics[0] == driverpos.Topics.UpdateNetworkRules && len(l.Data) >= 64 {
		...
		p.bs.DirtyRules, err = thankyoucoin.UpdateRules(p.bs.DirtyRules, diff)
		if err != nil {
