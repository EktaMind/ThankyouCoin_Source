...
func (p *EVMModule) Start(block blockproc.BlockCtx, statedb *state.StateDB, reader evmcore.DummyChain, onNewLog func(*types.Log), net thankyoucoin.Rules) blockproc.EVMProcessor {
...
func (p *PhotonEVMProcessor) Execute(txs types.Transactions, internal bool) types.Receipts {
	...
	receipts, _, skipped, err := evmProcessor.Process(evmBlock, p.statedb, thankyoucoin.DefaultVMConfig, &p.gasUsed, internal, func(log *types.Log, _ *state.StateDB) {
		p.onNewLog(log)
	})
	if err != nil {
