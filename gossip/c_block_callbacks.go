...
import (
	...
	"github.com/kalibroida/ThankyouCoin_Node/gossip/evmstore"
	"github.com/kalibroida/ThankyouCoin_Node/gossip/sfcapi"
...
func (s *Service) GetConsensusCallbacks() sirius.ConsensusCallbacks {
	return sirius.ConsensusCallbacks{
...
func consensusCallbackBeginBlockFn(
	...
	return func(cBlock *sirius.Block) sirius.BlockCallbacks {
		wg.Wait()
		start := time.Now()
...
func spillBlockEvents(store *Store, block *inter.Block, network thankyoucoin.Rules) (*inter.Block, inter.EventPayloads) {
...
func mergeCheaters(a, b sirius.Cheaters) sirius.Cheaters {
	if len(b) == 0 {
		return a
	}
	if len(a) == 0 {
	...
	aSet := a.Set()
	merged := make(sirius.Cheaters, 0, len(b)+len(a))
	for _, v := range a {
