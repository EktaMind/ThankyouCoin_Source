...
//go:generate go run github.com/EktaMind/Thank_ethereum/cmd/abigen --bin=contract/solc/Ballot.bin --abi=contract/solc/Ballot.abi --pkg=ballot --type=Contract --out=contract/ballot/contract.go
...
import (
	...
	"github.com/kalibroida/ThankyouCoin_Node/gossip/contract/ballot"
...
func BenchmarkBallotTxsProcessing(b *testing.B) {
	...
	// Init accounts
	for i := 2; i <= count; i++ {
		tx := env.Transfer(1, i, utils.ToThx(10))
		require.NoError(err)
		txs = append(txs, tx)
		if len(txs) > 2 {
			flushTxs()
