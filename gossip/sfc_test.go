...
//go:generate bash -c "cd ../../thankyoucoin-sfc && git checkout c1d33c81f74abf82c0e22807f16e609578e10ad8"
//go:generate bash -c "docker run --name go-thankyoucoin-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../thankyoucoin-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-thankyoucoin-sfc-compiler go-thankyoucoin-sfc-compiler-image && docker rm go-thankyoucoin-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../thankyoucoin-sfc:/src -w /src go-thankyoucoin-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../thankyoucoin-sfc:/src -w /src go-thankyoucoin-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../thankyoucoin-sfc && git checkout -- truffle-config.js; docker rmi go-thankyoucoin-sfc-compiler-image"
...
//go:generate go run github.com/EktaMind/Thank_ethereum/cmd/abigen --bin=./contract/solc/LegacySfcWrapper.bin --abi=./contract/solc/LegacySfcWrapper.abi --pkg=sfc100 --type=Contract --out=contract/sfc100/contract.go
...
//go:generate go run github.com/EktaMind/Thank_ethereum/cmd/abigen --bin=./contract/solc/NetworkInitializer.bin --abi=./contract/solc/NetworkInitializer.abi --pkg=netinit100 --type=Contract --out=contract/netinit100/contract.go
...
//go:generate go run github.com/EktaMind/Thank_ethereum/cmd/abigen --bin=./contract/solc/NodeDriver.bin --abi=./contract/solc/NodeDriver.abi --pkg=driver100 --type=Contract --out=contract/driver100/contract.go
...
//go:generate go run github.com/EktaMind/Thank_ethereum/cmd/abigen --bin=./contract/solc/NodeDriverAuth.bin --abi=./contract/solc/NodeDriverAuth.abi --pkg=driverauth100 --type=Contract --out=contract/driverauth100/contract.go
...
import (
	...
	"github.com/EktaMind/Thank_ethereum/common/hexutil"
	"github.com/EktaMind/Thank_ethereum/core/types"
	"github.com/stretchr/testify/require"

	"github.com/kalibroida/ThankyouCoin_Node/gossip/contract/driver100"
	"github.com/kalibroida/ThankyouCoin_Node/gossip/contract/driverauth100"
	"github.com/kalibroida/ThankyouCoin_Node/gossip/contract/netinit100"
	"github.com/kalibroida/ThankyouCoin_Node/gossip/contract/sfc100"
	"github.com/kalibroida/ThankyouCoin_Node/logger"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToThx(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToThx(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToThx(100))
		}
