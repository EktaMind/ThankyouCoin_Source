...
import (
	...
	"github.com/EktaMind/Thank_ethereum/accounts/abi/bind"
...
func newTestEnv() *testEnv {
	genStore := makegenesis.FakeGenesisStore(genesisStakers, utils.ToThx(genesisBalance), utils.ToThx(genesisStake))
...
func (env *testEnv) ApplyBlock(spent time.Duration, txs ...*types.Transaction) (receipts types.Receipts) {
	...
	process := beginBlock(&sirius.Block{
		Atropos: event.ID(),
	})
...
func (env *testEnv) callContract(
	...
	vmenv := vm.NewEVM(context, txContext, state, env.store.GetRules().EvmChainConfig(), thankyoucoin.DefaultVMConfig)
