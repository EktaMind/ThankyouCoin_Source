...
import (
	...
	"github.com/EktaMind/Thank_ethereum/console/prompt"
	"github.com/EktaMind/Thank_ethereum/ethclient"
	...
	"github.com/kalibroida/ThankyouCoin_Node/cmd/thankyoucoin/launcher/metrics"
	"github.com/kalibroida/ThankyouCoin_Node/cmd/thankyoucoin/launcher/tracing"
	"github.com/kalibroida/ThankyouCoin_Node/debug"
	"github.com/kalibroida/ThankyouCoin_Node/flags"
	"github.com/kalibroida/ThankyouCoin_Node/gossip"
	"github.com/kalibroida/ThankyouCoin_Node/integration"
	"github.com/kalibroida/ThankyouCoin_Node/utils/errlock"
	"github.com/kalibroida/ThankyouCoin_Node/valkeystore"
	_ "github.com/kalibroida/ThankyouCoin_Node/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-thankyoucoin"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-thankyoucoin command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, photonFlags...)
...
// thankyoucoin is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func siriusMain(ctx *cli.Context) error {
	...
	genesisPath := getPhotonGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.Photon.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.Photon.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.Photon, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local thankyoucoin node.
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call thx_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "thx_syncing")
				if err != nil {
					continue
				}
				if !syncing {
