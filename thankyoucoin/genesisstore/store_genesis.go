...
func (s *Store) GetRules() thankyoucoin.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &thankyoucoin.Rules{}).(*thankyoucoin.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg thankyoucoin.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() thankyoucoin.Genesis {
	meatadata := s.GetMetadata()
	return thankyoucoin.Genesis{
